...
StatusInfo ParseResponse(const std::string &text) {
            ...
            for (const auto &asset : assets) {
                ...
                if (!nameField.is_string() || !sizeField.is_number_unsigned() ||
                    ...
                    SPDLOG_WARN(
                        ...
                        nameField.type_name(), sizeField.type_name(), downloadUrlField.type_name());
                    continue;
                }
                ...
                SPDLOG_INFO(
                    "ParseResponse: Asset name: {}, size: {}, downloadUrl: {}.", name, size,
                    downloadUrl);
    ...
    #error "Need to port."
        ...
        SPDLOG_INFO(
            ...
            info.needToUpdate ? "Need to update" : "No need to update", info.localVer,
            info.latestVer);
        ...
        return std::make_pair(
            Status{Status::UpdateResponseJsonParseFailed}.SetAdditionalData(exception.what(), text),
            std::nullopt);
    }
}
} // namespace Impl
...
Status Info::DownloadAndInstall(const FnProgress &progressCallback) const {
    ...
    if (!tempPath.isValid()) {
        ...
        SPDLOG_WARN("Download: QTemporaryDir construct failed. error: {}", errorString);
    ...
    if (response.downloaded_bytes != fileSize) {
        ...
        return Status{Status::UpdateDownloadFileSizeMismatch}.SetAdditionalData(
            (size_t)response.downloaded_bytes, fileSize);
    }
...
StatusInfo Check() {
    ...
    if (response.status_code != 200) {
        ...
        return std::make_pair(
            Status{Status::UpdateResponseStatusCodeIsNot200}.SetAdditionalData(
                response.status_code, response.text),
            std::nullopt);
    }
